function[served_rate, transmit_power] = rate_allocation_simple1(vq, selectedaction, delay_r) 
...
% *channel_gain( distance_cal(a,b), h(a,b) ).^2
% Routing table
...
%                 constraints = [constraints, x_max(nsf, 1) == 0];
            ...
            constraints = [constraints, tx(nbs, nsf) <= 10];
        end
        if nbs >= 2
...
%                     replaced by the below constraints, 20170213
                    k = 1;                 
                    gain = channel_gain( distance_cal(bs_list1(n1),bs_list1(n1+1)), h(bs_list1(n1),bs_list1(n1+1)) ).^2;
                    constraints = [constraints, (1+ tx(bs_list1(n1), k)*gain ) >= exp(lowerbound(bs_list1(n1), k))   ];
